...
$query =    "SELECT a.created_at AS published, d.id, j.id AS project_id, j.user_id, d.customer_id, d.delivery_date, d.updated_at, d.type, (j.cost_location_id * 1) AS cost_location_id,
            YEAR(DATE(a.created_at)) year, MONTHNAME(DATE(a.created_at)) monthName, MONTH(DATE(a.created_at)) month,
...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $outerQuery =   "SELECT CONCAT(MONTHNAME(DATE(published)), ' ', YEAR(DATE(published))) AS label, year, month,
	                SUM(COALESCE(retail_price,0)) AS retail_price, SUM(COALESCE(buying_price,0)) AS buying_price, SUM(COALESCE(retail_price,0) - COALESCE(buying_price,0)) AS marge,
